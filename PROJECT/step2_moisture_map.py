...
def calculate_moisture(data, sea_level, sigma_ocean=100, sigma_rivers=40, noise_strength=0.4):
    ...
    moisture_ocean = gaussian_filter(
        (data <= sea_level).astype(float), sigma=sigma_ocean)

    river_mask = (data <= sea_level) & (
        gaussian_filter(land_mask.astype(float), sigma=2) > 0)
    moisture_rivers = gaussian_filter(
        river_mask.astype(float), sigma=sigma_rivers) * 3.0
    ...
    if noise_strength > 0:
        f_noise = fractal_noise(width, height, S=150,
                                octaves=4, persistence=0.5)
        f_noise = (f_noise - f_noise.min()) / (f_noise.max() - f_noise.min())
        moisture_map = (1 - noise_strength) * dist_moisture + \
            noise_strength * f_noise
    else:
...
def visualize_moisture(moisture_map, terrain_data, sea_level, save_path='step2_moisture_map.png'):
    ...
    moisture_cmap = mcolors.LinearSegmentedColormap.from_list(
        "moisture_map", moisture_colors)

    im2 = ax[1].imshow(moisture_map, cmap=moisture_cmap,
...
if __name__ == "__main__":
    ...
    moisture = calculate_moisture(terrain_data, SEA_LEVEL)
