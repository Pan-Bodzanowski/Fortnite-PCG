...
def generate_bridge_mask(terrain, roads_mask, sea_level):

    is_water = terrain <= sea_level
    raw_bridges = (roads_mask > 0) & is_water
    thick_bridges = binary_dilation(raw_bridges, iterations=1)
    
    return thick_bridges & is_water

def generate_tree_mask(biome_type_map, biome_name_to_type_id, terrain, cities_mask, roads_mask):
    H, W = terrain.shape
    ...
    forbidden_mask = (cities_mask > 0) | (roads_mask > 0)
    tree_mask[forbidden_mask] = 0
...
def visualize_all(terrain, biome_type_map, biome_name_to_type_id,
                  tree_mask, bridge_mask, sea_level, save_path):
    ...
    bridge_overlay = np.zeros((H, W, 4))
    bridge_overlay[bridge_mask > 0] = [0.6, 0.3, 0.0, 1.0]  # Brązowe mosty
    ax.imshow(bridge_overlay)

    tree_overlay = np.zeros((H, W, 4))
    tree_overlay[tree_mask > 0] = [0.0, 0.25, 0.0, 0.9]
    ax.imshow(tree_overlay)

    ax.set_title("Biomes, Trees and Bridges", fontsize=16)
    ax.axis('off')
    ...
    legend_patches.append(Patch(color=(0.6, 0.3, 0.0, 1.0), label="Bridges"))
...
def main():
    ...
    roads_mask = np.load(ROADS_MASK_FILE)
    ...
    tree_mask = generate_tree_mask(biome_type_map, biome_name_to_type_id,
                                   base_terrain, cities_mask, roads_mask)

    bridge_mask = generate_bridge_mask(base_terrain, roads_mask, SEA_LEVEL)

    np.save(OUTPUT_BIOME_TYPE_MAP, biome_type_map)
    np.save(OUTPUT_TREE_MASK, tree_mask)
    np.save(OUTPUT_BRIDGE_MASK, bridge_mask)

    visualize_all(base_terrain, biome_type_map, biome_name_to_type_id,
                  tree_mask, bridge_mask, SEA_LEVEL, OUTPUT_IMAGE)
